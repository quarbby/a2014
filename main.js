...
// Build list of positions to traverse
...
GameManager.prototype.findFarthestPosition = function (cell, vector) {
  ...
  // Progress towards direction until obstacle found
  do {
    previous = cell;
  ...
  return {
    farthest: previous,
    next: cell // To check if merge required
  };
};
...
// First available random position
...
HTMLActuator.prototype.actuate = function (grid, metadata) {
  ...
  window.requestAnimationFrame(function () {
    ...
    if (metadata.won) { self.message(true); } // Win!
  });
};
...
HTMLActuator.prototype.addTile = function (tile) {
  ...
  if (tile.previousPosition) {
    // Render tile in previous pos
    window.requestAnimationFrame(function () {
      classes[2] = self.positionClass({ x: tile.x, y: tile.y });
      self.applyClasses(element, classes); // Update pos
    });
